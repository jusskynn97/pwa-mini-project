...
async function initCapacitorPlugins() {
    IS_CAPACITOR = typeof window !== 'undefined' && !!window.Capacitor;

    try {
        if (window.Capacitor?.Plugins) {
            capacitorPlugins.Camera = window.Capacitor.Plugins.Camera || null;
            capacitorPlugins.Network = window.Capacitor.Plugins.Network || null;
            capacitorPlugins.Geolocation = window.Capacitor.Plugins.Geolocation || null;
            capacitorPlugins.LocalNotifications = window.Capacitor.Plugins.LocalNotifications || null;
        }
    } catch (e) {
        console.warn('[Capacitor] Could not load plugins from global:', e);
    }

    console.log('[Capacitor] Native mode:', IS_CAPACITOR, '| Plugins:', {
        Camera: !!capacitorPlugins.Camera,
        Network: !!capacitorPlugins.Network,
        Geolocation: !!capacitorPlugins.Geolocation,
        LocalNotifications: !!capacitorPlugins.LocalNotifications
    });
}

async function sendLocalNotification(title, body, id = null, extra = {}) {
    if (!capacitorPlugins.LocalNotifications) return false;
    try {
        await capacitorPlugins.LocalNotifications.schedule({
            notifications: [{
                id: id || Math.floor(Math.random() * 2147483647),
                title,
                body,
...
function base64ToDataUrl(base64, format = 'image/jpeg') {
    if (!base64) return '';
...
async function loadDraft() {
    ...
    if (data.latitude && data.longitude) {
        appState.formData.latitude = data.latitude;
        appState.formData.longitude = data.longitude;
        appState.formData.locationAccuracy = data.locationAccuracy;
        const statusEl = document.getElementById('gpsStatus');
        const coordsEl = document.getElementById('gpsCoords');
        const accEl = document.getElementById('gpsAccuracy');
        if (statusEl) statusEl.classList.remove('hidden');
        if (coordsEl) coordsEl.textContent = `Vĩ độ: ${data.latitude.toFixed(6)}, Kinh độ: ${data.longitude.toFixed(6)}`;
        if (accEl) accEl.textContent = `Độ chính xác: ±${Math.round(data.locationAccuracy || 0)}m`;
    }
...
function updateNetworkStatus(forceStatus = null) {
    appState.online = forceStatus !== null ? forceStatus : navigator.onLine;
    ...
    if (appState.online) {
        ...
        sendLocalNotification('Mất kết nối mạng', 'Ứng dụng đang chạy Offline - Dữ liệu sẽ được lưu tự động', 1001);
    }
}

async function setupCapacitorNetwork() {
    if (!capacitorPlugins.Network) return;
    try {
        const status = await capacitorPlugins.Network.getStatus();
        updateNetworkStatus(status.connected);

        await capacitorPlugins.Network.addListener('networkStatusChange', (s) => {
            console.log('[Network] Status changed:', s);
            updateNetworkStatus(s.connected);
        });
        console.log('[Network] Capacitor Network listener attached');
    } catch (e) {
        console.warn('[Network] Setup failed, falling back to navigator.onLine:', e);
    }
}
...
async function sendToGoogleSheets(inspection) {
    ...
    const payload = {
        ...
        locationAccuracy: inspection.locationAccuracy || null
    };
...
async function compressAndShowPhoto(base64OrFile, source = 'web') {
    try {
        let dataUrl;
        if (typeof base64OrFile === 'string') {
            dataUrl = base64ToDataUrl(base64OrFile, 'image/jpeg');
        } else {
        ...
        if (source !== 'web') {
            ...
            sendLocalNotification('Đã chụp ảnh', 'Ảnh đã được lưu vào thư viện và đính kèm báo cáo', 1010);
        }

        showToast('Đã thêm ảnh vào báo cáo', 'success');
    } catch (err) {
        console.error('[Photo] Process failed:', err);
        showToast('Không thể xử lý ảnh: ' + (err.message || err), 'error');
    }
}
    ...
    if (capacitorPlugins.Camera) {
        try {
            ...
            if (photo?.base64String) {
                await compressAndShowPhoto(photo.base64String, 'camera');
            } else {
                showToast('Không nhận được dữ liệu ảnh', 'error');
            }
        } catch (e) {
            console.warn('[Camera] Take photo failed:', e);
            if (e.message && /cancel|user|denied/i.test(e.message)) {
                showToast('Bạn đã hủy chụp ảnh hoặc từ chối quyền camera', 'warning');
            } else {
                showToast('Lỗi camera: ' + (e.message || e), 'error');
            }
        }
    } else {
...
async function pickImage() {
    if (capacitorPlugins.Camera) {
        try {
            showToast('Đang mở thư viện ảnh...', 'info', 1500);
            const photo = await capacitorPlugins.Camera.getPhoto({
                quality: 85,
                allowEditing: false,
                resultType: 'base64',
            ...
            if (photo?.base64String) {
                await compressAndShowPhoto(photo.base64String, 'gallery');
            } else {
                showToast('Không nhận được dữ liệu ảnh', 'error');
            }
        } catch (e) {
            console.warn('[Camera] Pick image failed:', e);
            if (e.message && /cancel|user|denied/i.test(e.message)) {
                showToast('Bạn đã hủy chọn ảnh hoặc từ chối quyền truy cập', 'warning');
            } else {
...
async function getGpsLocation() {
    const onSuccess = (lat, lng, acc) => {
        appState.formData.latitude = lat;
        appState.formData.longitude = lng;
        appState.formData.locationAccuracy = acc;

        const statusEl = document.getElementById('gpsStatus');
        const coordsEl = document.getElementById('gpsCoords');
        const accEl = document.getElementById('gpsAccuracy');
        statusEl.classList.remove('hidden');
        coordsEl.textContent = `Vĩ độ: ${lat.toFixed(6)}, Kinh độ: ${lng.toFixed(6)}`;
        accEl.textContent = `Độ chính xác: ±${Math.round(acc)}m`;

        showToast(`Đã lấy vị trí (±${Math.round(acc)}m)`, 'success');
        sendLocalNotification('Đã lấy vị trí GPS', `Tọa độ: ${lat.toFixed(5)}, ${lng.toFixed(5)}`, 1005);
    };

    const onError = (err) => {
        console.warn('[Geolocation] Error:', err);
        showToast('Không thể lấy vị trí: ' + (err.message || err), 'error');
    };

    if (capacitorPlugins.Geolocation) {
        try {
            showToast('Đang lấy vị trí GPS...', 'info', 2500);
            await capacitorPlugins.Geolocation.requestPermissions?.();
            const pos = await capacitorPlugins.Geolocation.getCurrentPosition({
                enableHighAccuracy: true,
                timeout: 15000
            });
            onSuccess(pos.coords.latitude, pos.coords.longitude, pos.coords.accuracy || 0);
        } catch (e) {
            onError(e);
        }
    } else if ('geolocation' in navigator) {
        showToast('Đang lấy vị trí...', 'info', 2500);
        navigator.geolocation.getCurrentPosition(
            (pos) => onSuccess(pos.coords.latitude, pos.coords.longitude, pos.coords.accuracy || 0),
            (err) => onError(err),
...
function clearGpsLocation() {
    appState.formData.latitude = null;
    appState.formData.longitude = null;
    appState.formData.locationAccuracy = null;
    document.getElementById('gpsStatus')?.classList.add('hidden');
    showToast('Đã xóa vị trí GPS', 'info');
}
...
async function manualSync() {
    ...
    if (successCount > 0) {
        sendLocalNotification(
            'Đồng bộ thành công',
            `${successCount} báo cáo đã được gửi lên Google Sheets`,
            1030
        );
    }
    if (failCount > 0) {
        sendLocalNotification(
            'Đồng bộ gặp lỗi',
            `${failCount} báo cáo thất bại, vui lòng thử lại sau`,
            1031
        );
    }
}
...
async function handlePhotoUpload(e) {
    ...
    await compressAndShowPhoto(file, 'web');
    e.target.value = '';
}
...
function renderSummary() {
    ...
    summaryEl.innerHTML = `
        ...
        <div class="summary-item">
            <span class="summary-label">📍 Vị trí GPS</span>
        ...
        ${data.latitude && data.longitude ? `
            <div class="summary-item" style="flex-direction: column; align-items: flex-start; gap: 4px;">
                <span class="summary-label">🧭 Độ chính xác</span>
                <span class="summary-value" style="font-size: 13px;">±${Math.round(data.locationAccuracy || 0)} mét</span>
            </div>
        ` : ''}
        <div class="summary-item">
...
function resetForm() {
    ...
    document.getElementById('gpsStatus')?.classList.add('hidden');
    document.getElementById('photoSaveStatus')?.classList.add('hidden');
...
async function submitInspection() {
    ...
    if (appState.online && CONFIG.GOOGLE_SHEETS.ENABLED) {
        sendLocalNotification('Báo cáo đã lưu', `Đang gửi ${inspection.building}-${inspection.roomNumber} lên Google Sheets...`, 1020);
        await requestBackgroundSync();
        setTimeout(() => manualSync(), 1000);
    } else if (appState.online) {
        showToast('Đã lưu - Google Sheets chưa cấu hình', 'info');
        sendLocalNotification('Báo cáo đã lưu', `${inspection.building}-${inspection.roomNumber} đã lưu cục bộ`, 1020);
    } else {
        showToast('Đã lưu offline - Sẽ tự động đồng bộ khi online', 'info');
        sendLocalNotification('Đã lưu Offline', `${inspection.building}-${inspection.roomNumber} sẽ tự động đồng bộ khi có mạng`, 1021);
        await requestBackgroundSync();
    }
...
function setupFormInputs() {
    ...
    document.getElementById('gpsRemove')?.addEventListener('click', clearGpsLocation);
...
async function init() {
    ...
    window.addEventListener('online', () => {
        if (!capacitorPlugins.Network) updateNetworkStatus(true);
    });
    window.addEventListener('offline', () => {
        if (!capacitorPlugins.Network) updateNetworkStatus(false);
    });

    if (capacitorPlugins.LocalNotifications) {
        try {
            await capacitorPlugins.LocalNotifications.requestPermissions();
            console.log('[LocalNotifications] Permission requested');
        } catch (e) {
    ...
    if (IS_CAPACITOR) {
        addSyncLog('Running as Capacitor Native App', 'info');
    }
